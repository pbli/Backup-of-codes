...
/* WHYWHYWHYWHYWHY
     1.      9axis MPU9250
     1.1 read 9 axis raw data and perform fusion by myself based on complementaty filter
         very much depends on calibration of compass, the yaw will not stable and use a lot power (high rate)
     1.2 perform fusion based on DMP, the fusion of mag also need used develpment and it will also very sensitive to disturbance, 
         the work will hard and the result will be not very good. also the power will be a problem
	 1.3 just perform 6 axis fusion with dmp. the power of compass is waste
      
	 So just use 6050,we could get good data and save the most power
*/
...
void mpu_init_structures()
{
    ...
    switch (deviceIndex) {
      case 0:
        hw->addr = 0xD2;//address for imu for my i2c program
        break;
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (mpu_set_lpf(42))
        return -12;
    if (mpu_set_sample_rate(200))//sample rate of MPU data in order to smaller delay
        return -13;
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias( short *accel_bias)
{
    ...
    if (i2c_read(st->hw->addr, 0x06, 6, data))//original offsets
        return -1;
    ...
    accel_hw[0] += got_accel[0];//add new
    accel_hw[1] += got_accel[1];
    accel_hw[2] += got_accel[2];
	
	//accel_hw[0]=got_accel[0]+accel_bias[0];
	//accel_hw[1]=got_accel[1]+accel_bias[1];
	//accel_hw[2]=got_accel[2]+accel_bias[2];
	

    data[0] = (accel_hw[0] >> 8) & 0xff;//high
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    switch (fsr) {
    case 250:
        data = INV_FSR_250DPS << 3;//0
        break;
    case 500:
        data = INV_FSR_500DPS << 3;//01
        break;
    case 1000:
        data = INV_FSR_1000DPS << 3;//10
        break;
    case 2000:
        data = INV_FSR_2000DPS << 3;//11
        break;
    default:
        return -1;
    }
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st->chip_cfg.lpf == data)
        return 0;
    if (i2c_write(st->hw->addr, st->reg->lpf, 1, &data))//set dlpf
        return -1;
